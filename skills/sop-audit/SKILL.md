...
6. **出双轨报告**:
   ...
   - **(b) 可执行 findings**(owner 说"改"时据此动手):
     ...
     ```
7. **收尾卡**(任务收口卡在 audit 里的落地,轻量、别写长):
...
8. **若 owner 说"改 / go"** → 选最短安全路径:低风险可逆的本地修正在当前树直接改 + 验证 + change-first review;需远端交付 / 评审才开分支 + PR;需跨会话跟踪才开 issue;真并行才用 worktree。高风险始终回 owner。**没说"改"就停在第 7 步。**
