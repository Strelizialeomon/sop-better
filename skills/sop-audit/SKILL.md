---
name: sop-audit
description: Use when 用户要体检/审计/优化现有开发 SOP,怀疑 SOP 太重、结构错配、反驳缺失、结构缺失、凭据失真,或需要检查 Codex AGENTS.md 与旧 CLAUDE.md 残留。
---
...
4. **按 §5 五类出 finding,每条标 severity + 证据**。**查法细则全在 STANDARD §5(第 1 步已读),本技能不重抄(重抄必漂移),这里只定 severity 映射**:
   ...
   - **P2 结构错配**= §5.2(含 ④删除残留 · kind `mismatch`/`stale`);**P2 沟通约束塌成口号 / 过度阻塞**= §5.3(反驳 / 说人话 / 诚实无知闸 / 三层引导 / 方案前调研包 / 任务收口卡)。
