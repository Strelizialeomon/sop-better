...
4. **主线覆盖闸(防漏停顿点)**:在输出 findings 前,必须先填 6 行表;这是覆盖检查,不是第六类 finding。**无权威锚点不得填覆盖**:每行都要先指向当前 `STANDARD.md` / `master/` 母本 / `skills/sop-audit/SKILL.md` 的对应规则,再指向目标项目落点;只在目标项目里搜到近似关键词,不能算覆盖。
   ```md
   ## 主线覆盖闸
   ...
   ```
   - **覆盖最小口径**:Refs/Closes = 默认 `Refs`、只有满足验收/关闭条件才 `Closes`;三件套 = doc 真相源、issue 消息总线、PR 交付验证互不替代;issue 评论分层 = 决策/实测/收口/状态校正/阻塞或高风险/残项迁移/路线变化不能短评;查证闭环 = 本地事实+外部事实+未验风险,且不把小事拖成过度确认;高风险治理项 = 治理 doc / AGENTS / 协作约定 / 跨端契约或骨架变更回 owner 人审;结构触发 = 端/人/并行 agent 分开判(第 2 个端触发 contracts/端级身份文档,真并行才触发 worktree/协调骨架,第 2 个人触发 handoff)。
   - **状态处理**:`覆盖 / 匹配` 不进 findings;`缺失 / 预建 / 过重` 进入 findings;`偏离` 必须一句话判断合理与否,不合理才进 findings。表固定 6 行,新增主线必须替换旧项,不能追加成清单膨胀。
