...
4. **按 §5 四类出 finding,每条标 severity + 证据**:
   ...
   - **P3 凭据失真 / 交接断裂**:issue/PR 烂尾(开着没人管)、状态与实际不符、issue 厚 doc 薄(细节堆 issue 难被 agent 复用);**issue 指向的 doc 在远端解析不开(悬空链接)、SOP 缺「生产先落 doc 再开 issue / 消费先验链接」的交接闸、或约束块缺「缺交付物反弹不自补」的角色边界 carve-out** → 凭据不可信,agent 会照空链接行动甚至自己伪造需求(STANDARD §1.8 + §1.9)。
