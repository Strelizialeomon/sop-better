...
5. **没有的端/人不预建**(右尺寸硬约束):单端不建 contracts/按端操作台/worktree;单人不建协作 doc;真一次性脚本压根不跑 `$sop-init`(出范围)。
