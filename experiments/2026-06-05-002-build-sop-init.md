...
- **超出预期 / 想不到的地方**(待 owner 填):
  > （你来写）
- **翻车 / 要纠偏**:
  > owner 抓到:「顶嘴/反驳」当时只写成**口号**(STANDARD 一句原则 + 模板一行字),没写成机制。已纠偏 → 升级成「**顶嘴协议**」6 闸(强制输出 + 闸门,可检查)。**反例是 AI 自己**:写完"禁止讨好"下一条还在说"正中靶心"——铁证口号无效,必须做成可检查的闸。(commit: 顶嘴协议化)
  > owner 抓到第二处:SKILL.md 只写进 repo,**没软链进 `~/.claude/skills/`**,Claude Code 扫不到 → `/sop-init` 不存在。"造了≠装了"。已修:按 google-design-skill 的软链装法装上 + SKILL 引用改绝对路径(软链后相对路径会断)。教训沉淀:**AI 的"完成"定义必须含"真能被调用/被跑",否则就是又一个口号。**
  > （其余待 owner 填:还有别的想毙/别扭吗?)
