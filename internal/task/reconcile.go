...
func (reconciler Reconciler) Transition(ctx context.Context, claim StoredClaim, target BusinessState, reason string, evidence *CompletionEvidence) (ReconcileResult, error) {
	...
	serverTime, _, err := reconciler.Claims.ServerNow(ctx)
	if err != nil {
		return ReconcileResult{}, fmt.Errorf("read GitHub server time for state event: %w", err)
	}
	event := StateEvent{
		EventID: eventID, StateRevision: issue.Revision + 1, ExpectedPreviousRevision: issue.Revision,
		RunID: claim.Claim.RunID, LeaseEpoch: claim.Claim.LeaseEpoch, FencingToken: claim.Claim.FencingToken,
		SourceActor: "runtime-runner", SourceServerTime: serverTime,
		From: issue.State, To: target, Reason: strings.TrimSpace(reason), Evidence: evidence,
	}
	updated, err := reconciler.Issues.AppendStateEvent(ctx, claim.Claim.RepoNodeID, claim.Claim.IssueNumber, event)
...
func (reconciler Reconciler) Reconcile(ctx context.Context, request ReconcileRequest) (ReconcileResult, error) {
	...
	if issue.State == StateReady && active {
		return reconciler.appendState(ctx, request, issue, claim, StateRunning, "claim-created")
	}
	...
	if issue.State == StateRunning && !active {
		result, err := reconciler.appendState(ctx, request, issue, StoredClaim{}, StateWaiting, "missing-active-claim")
		if err != nil {
			return ReconcileResult{}, err
		}
		if claimExists {
