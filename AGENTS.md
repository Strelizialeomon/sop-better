...
本仓 = `SOP_HOME`,产出 `$sop-init`、`$sop-audit` 两个 Codex-first 技能(Claude Code 仅保留桥接兼容)。**唯一真相源是 `STANDARD.md`**——改任何规则先读它;其 §1 公约(反驳协议、凭据保真等)对本仓会话同样生效。

- **软链即线上**:`skills/` 软链进 `~/.codex/skills/`(主)和 `~/.claude/skills/`(桥接),**工作树就是线上**——改完即生效,切分支 = 换线上版本;干完活回 main。
