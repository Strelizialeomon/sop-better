...
## 8. 再增补(同次 go · owner 追加,源自反例 geo-reverse #2)

两条,继续瘦身:

1. **业务必须确认"产品形态"(治 §1.2 的洞)**:反例 #2——不懂技术的需求方"确认"出了命令行。病根:形态属"要什么"、该业务定,但 SOP 的"要什么"只列了目标/约束/验收、漏了形态 → 滑给开发顺手的默认(CLI)。
   - 落点(全是 +0 行内追加):`STANDARD.md` §1.2 加"形态属要什么、别默认 CLI + 反例";§1.3 收口标准 三样→**四样(+形态)**;`agent-constraints.md` 标准块业务角色行 +形态;`collaboration.md` 收口标准 +④形态、req-doc 内容 +形态。
2. **开发取活先走"要不要细化"一轮(agent 协助开发)**:别抓起 issue 就闷头写。够清楚直接干 / 不清楚先拆 / 缺业务该给的"要什么"(含形态)→ 反弹业务(复用 §1.9 carve-out,不自造)。
   - 落点:`issue-pr-workflow.md` 消费细则 +1 条(canonical,C1/C2 共用);`agent-constraints.md` carve-out 行 +半句(always-loaded)。

**不做**:不给 sop-audit 加"缺形态确认"查法(§1.2+收口标准带上即足);不在 c2 6+1/req-doc 多处重列(收口标准是单一真相源)。**净增 ≤ 5 行**。**点 2/3(gh + 角色)上一轮 f868b60 已交付。**
