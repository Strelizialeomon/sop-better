...
低风险单会话小改照旧走「什么时候用」豁免,不进闸。适用任务按下表走:**每完成一步 → 勾 issue 流程格 → 报"本步完成 + 下一步是 X",停等 owner 一字放行**;owner 说"连走到 X" = 中间闸静默通过,只在 X 报到。**高风险闸独立**,不随步闸放行。
...
**双状态锚(主从定死)**:issue 正文的「流程」勾选框(建 issue 用 `.github/ISSUE_TEMPLATE/` 模板自带)是**真相源**——跨会话 / 换 agent 打开 issue 即定位;接活第一动作把 6 步展开成会话任务清单当**投影**逐步勾。两处不一致以 issue 为准;需求变更要回退步骤 → 评论说明 + 改勾,不许静默回拨(凭据保真)。
