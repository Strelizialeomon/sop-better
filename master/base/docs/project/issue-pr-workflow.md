...
低风险单会话小改照旧走「什么时候用」豁免,不进闸。适用任务按下表走:**每完成一步 → 勾 issue 流程格 → 报"本步完成 + 下一步是 X",停等 owner 一字放行**;owner 说"连走到 X" = 中间闸静默通过,只在 X 报到。**高风险闸独立**,不随步闸放行。(步进闸是 owner 显式订购的放行节奏,不算 §1.9 的阻塞 / 假民主;**步内**仍照 §1.9 自决。)
