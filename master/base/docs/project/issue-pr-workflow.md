...
| 3 验证+复审 | 跑验收命令;新眼睛 review;修 ①(第 2 轮起 reviewer 只核修复) | 验收全过 + ① 档归零(② 档已上报 · 分档与轮数见根 `CLAUDE.md`;2 轮后仍有余 ① → 按表下遇阻注记上交,owner 拍板后本步才算完) | "验证/review 过(证据贴附)。下一步交付 PR,继续?"(余 ① 上交场景不用本话术,走遇阻报法) |
...
**遇阻停 ≠ 过闸停(任何一步都可能撞上,不只第 2 步)**:步内撞上自己解决不了的东西 → 按 §1.9「遇阻处置」判档,别套本步的过闸话术(第 3 步 2 轮上限后的余 ① 上交也走本条报法)。两种报法开头就分得出——过闸报「本步完成,下一步 X,继续?」(owner 回一个字即可);遇阻报「**卡在第 N 步**:〈卡点 + 下一条路为什么走不了〉;需要你给〈什么〉;已做的在〈哪〉」(owner 必须给内容,回"go"救不了)。**遇阻停与高风险闸同列**:不随步闸放行,也不被「连走到 X」覆盖。
