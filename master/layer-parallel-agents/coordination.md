...
> 每端的**身份 + 边界**写在各端 `<端目录>/AGENTS.md`（端级文档 · `$sop-init` 按 `ends[]` 生成 · Codex 自动加载）。此处只给跨端协作骨架，**不复述各端边界**（单一真相源 · STANDARD §1.7）。
...
6. **收口**：别把"做完了"手抄进多个 doc（单一真相源 · STANDARD §1.7）。**owner 说"收工"= 推收口 doc 远端的明确指令（文档 main / 代码 PR）**。
